...
One semantic model can support projections for different needs. The canonical semantic model is projected into an appropriate bounded representation, which may then be delivered to intelligent systems as a Semantic Twin, JSON, an API or MCP response, or another machine-facing representation of the shared semantic model. Markdown with YAML front matter is the v0.1 reference representation because it is inspectable, easy to publish, easy to generate, and legible to current language models.
